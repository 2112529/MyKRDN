...
def get_neg_one(user,args,n_items,train_user_set):
    user = int(user)
    each_negs = list()
    # print(args)
...
def get_feed_data(train_entity_pairs, train_user_set,args,n_items):
    def negative_sampling(user_item, train_user_set,args,n_items):
        pool = multiprocessing.Pool(cores)
        neg_items = pool.starmap(get_neg_one, [(user, args,n_items,train_user_set) for user in user_item.cpu().numpy()[:, 0]])
        pool.close()
        return neg_items
    ...
    feed_dict['neg_items'] = torch.LongTensor(negative_sampling(entity_pairs, train_user_set, args,n_items))
    return feed_dict
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epoch):
        ...
        if epoch%20 == 1 or epoch==0:
            ...
            all_feed_data = get_feed_data(train_cf_pairs, user_dict['train_user_set'],args,n_items)  # {'user': [n,], 'pos_item': [n,], 'neg_item': [n, n_sample]}
